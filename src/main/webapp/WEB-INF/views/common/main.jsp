...
$(function(){
	// 최근 등록된 공지글 3개 전송받아서 출력 처리
		...
		success: function(data){
			...
			for(var i in json.nlist){
				values += "<tr><td>" + json.nlist[i].no + "</td><td><a href='/first/ndetail?no="
						+ json.nlist[i].no + "'>" 
	...
	$.ajax({
		...
		success: function(data){
			console.log("success : " + data);
			console.log(data.list[0].bnum + ", " + data.list[0].btitle + ", " + data.list[0].rcount);
			...
			for(var i in json.list){
				values += "<tr><td>" + json.list[i].bnum + "</td><td><a href='/first/bdetail?bnum="
						+ json.list[i].bnum + "'>" 
